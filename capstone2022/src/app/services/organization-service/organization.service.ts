...
export class OrganizationService {
  ...
  getPositionByOrgID(id: number) {
    return this.__http.post(
      `https://localhost:44376/api/ProfileAPI/GetListPositionByOrgID?ID=${id}`,
      []
    );
  }
  ...
  insertPosition(request: any) {
    return this.__http.post(this.baseUrl + '/InsertPosition', request);
  }
  modifyPosition(request: any) {
    return this.__http.post(this.baseUrl + '/ModifyPosition', request);
  }

  deletePosition(arr: Array<number>) {
    return this.__http.post(this.baseUrl + '/DeletePosition', arr);
  }
}
